...
object SbtDistPlugin extends Plugin {
  ...
  def distTask = (distFiles, distPath, distTeamCityIntegration, streams) map { (src, dest, teamCityIntegration, s) =>
  ...
  def filesOnDistClasspath = (managedClasspath in dist, target, streams) map { (cp, t, s) =>
  ...
  val distSettings: Seq[Project.Setting[_]] = Seq(
    ...
    distFiles <<= filesOnDistClasspath,
